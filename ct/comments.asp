...
<body bgcolor="#F2F2F2" style="margin-top: 2pt; margin-left:2px; margin-right:2px;">
<P>
...
' July 2005, using index tables 'RestictedComments' and 'HB_Membership'
  ...
  strSQL = _ 
     "SELECT C.CommentID, C.RPId, C.CommentByUserID, C.CommentByUser_Particulars, C.CommentDate, C.CommentToPageNo, C.CommentToPlace, C.Comment, (SELECT TOP 1 RC.ID FROM RestrictedComments RC WHERE RC.CommentID = C.CommentID) AS RestrictedID " &_
     "FROM Comments C " &_
     "WHERE RPId = " & iRPId
...
'  Response.Write "strCommenterParticulars=" & strCommenterParticulars & ";<br>"   ' DEVELOPMENT & DEBUG
  ...
  strComment = ""
  strCommentToPageNo = LCase(Trim(CommentRS("CommentToPageNo")))
  ' Response.Write "strCommentToPageNo=" & strCommentToPageNo & "<br>"        ' DEVELOPMENT & DEBUG
  ' Response.End          ' DEVELOPMENT & DEBUG

  If strCommentToPageNo <> "" Then
    If strCommentToPageNo <> "all" Then
      strComment = "Page(s) " & strCommentToPageNo & ", "
    End If
  End If
  If CommentRS("CommentToPlace") <> "" Then
    ...
    strComment = strComment & CommentRS("CommentToPlace") & ":" & chr(13) & chr(10)
  End If
